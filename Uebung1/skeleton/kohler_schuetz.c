...
/* TODO Task (h) complete printInstruction here */    
    Operation operation = operations[i->i.opcode];
    Function function = functions[i->r.funct];

    switch (operation.type) {
        case iType:
            printf("%-4s %02i, %02i, 0x%04x\n", operation.name, i->i.rt, i->i.rs, i->i.immediate);
            break;
                                            
        case jType:
            printf("%-4s 0x%08x\n", operation.name, i->j.address);
            break;
        
        case rType:
            printf("%-4s %02i, %02i, %02i, 0x%04x\n", function.name, i->r.rd, i->r.rs, i->r.rt, i->r.shamt);
            break;
